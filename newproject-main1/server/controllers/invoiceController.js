...
function loadFallbackDb() {
  ...
  if ((process.env.VERCEL || process.env.NODE_ENV === 'production') && !fs.existsSync(FALLBACK_DB_PATH)) {
    try {
      ...
      console.error('[Fallback DB invoiceController.js] Failed to initialize /tmp fallback:', err.message);
    }
  }
...
function saveFallbackDb(data) {
  try {
    ...
    console.error('[Fallback DB invoiceController.js] Write failure:', err.message);
  }
}
